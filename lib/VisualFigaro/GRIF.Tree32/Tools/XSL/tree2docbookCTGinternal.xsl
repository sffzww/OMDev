...
<xsl:template match="sif" mode="sif-body">
	<article>
		...
		<xsl:variable name="big" select="/export/pages/page[@idx=$var-page-idx]/tool[@type='grp']/@big"/>
		
		<para align="right">
		<xsl:if test="$big='false'"> <!--  on peut mettre les 2 images à coté -->
			<informaltable  align="center" frame ="void" border="000000"> <tgroup cols="2"> 
				<colspec colnum="1" colwidth="1*"/>
				<colspec colnum="2" colwidth="1.5*"/>
				<tbody>
					<row>
						<entry align="center" valign="middle" colsep="0">
							...
							<xsl:call-template name="tPicture">
								...
								<xsl:with-param name="pScale" select="16.5"/>
							</xsl:call-template>
						</entry>
						<entry  align="center" valign="middle" colsep="0">
							<!-- Curve picture -->
							<xsl:call-template name="tPicture">
								<xsl:with-param name="pFileRef" select="/export/pages/page[@idx=$var-page-idx]/courbes/courbes-displayed[@leg='normal']/@img"/>
								<xsl:with-param name="pScale" select="55"/>
							</xsl:call-template>
						</entry>
			        </row>
				</tbody>
		...
		<xsl:if test="$big='true'"> <!--  on ne peut pas mettre les 2 images à coté, on fait dessus dessous -->
		<informaltable  align="center" frame ="void" border="000000"> <tgroup cols="1"> 
			<colspec colnum="1" colwidth="1*"/>
			<tbody>
				<row>
					<entry align="center" valign="middle" colsep="0">
	        	...
	        	<row>
					<entry  align="center" valign="middle" colsep="0">
						...
						<xsl:call-template name="tPicture">
							<xsl:with-param name="pFileRef" select="/export/pages/page[@idx=$var-page-idx]/courbes/courbes-displayed[@leg='normal']/@img"/>
							<xsl:with-param name="pScale" select="45"/>
						</xsl:call-template>
					</entry>
				</row>
			</tbody>
			...
			<tgroup cols="5"> 
				...
				<thead>
				  <row>
					...
					<entry><xsl:value-of select="document($resfile)/labels/label[@key='otherparameters']/@val"/></entry>
				  </row>
				</thead>
				<tbody>
					<!-- La première colonne contient le nom de la partie  -->
...
<xsl:template name="tDisplayActuatorSensorInTable">
	...
	<xsl:param name="pFirst"/>
		...
		<row>
			...
			<entry>
				<xsl:call-template name="tDisplayPropertiesActuSensorOtherParameters">
					<xsl:with-param name="pComp" select="$pComp"/>
				</xsl:call-template>
			</entry>
		</row>
...
<xsl:template name="tDisplaySolverInTable">
	<xsl:param name="pComp"/>
		...
		<row>
			...
			<entry>
				<xsl:call-template name="tDisplayPropertiesSolverAdvanced">
					<xsl:with-param name="pComp" select="$pComp"/>
				</xsl:call-template>
			</entry>
		</row>
