...
set(Qt5XmlPatterns_VERSION_STRING 5.6.0)
...
macro(_populate_XmlPatterns_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5XmlPatterns_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::XmlPatterns PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5XmlPatterns_LIB_DEPENDENCIES}${_list_sep}${_Qt5XmlPatterns_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5XmlPatterns_LIB_DEPENDENCIES}${_list_sep}${_Qt5XmlPatterns_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::XmlPatterns)
    ...
    set(Qt5XmlPatterns_PRIVATE_INCLUDE_DIRS
        "${_qt5XmlPatterns_install_prefix}/include/QtXmlPatterns/5.6.0"
        "${_qt5XmlPatterns_install_prefix}/include/QtXmlPatterns/5.6.0/QtXmlPatterns"
    )
    ...
    foreach(_module_dep ${_Qt5XmlPatterns_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5XmlPatterns_FIND_VERSION_EXACT}
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5XmlPatterns_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5XmlPatterns_PLUGINS)
            if (matched)
    ...
    macro(_populate_XmlPatterns_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        set(imported_location "${_qt5XmlPatterns_install_prefix}/share/qt5/plugins/${PLUGIN_LOCATION}")
        _qt5_XmlPatterns_check_file_exists(${imported_location})
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5XmlPatterns_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_XmlPatterns_target_properties(RELEASE "Qt5XmlPatterns.dll" "libQt5XmlPatterns.dll.a" )



    _populate_XmlPatterns_target_properties(DEBUG "Qt5XmlPatternsd.dll" "libQt5XmlPatternsd.dll.a" )
