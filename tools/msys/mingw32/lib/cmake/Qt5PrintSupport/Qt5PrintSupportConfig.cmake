...
set(Qt5PrintSupport_VERSION_STRING 5.6.0)
...
macro(_populate_PrintSupport_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5PrintSupport_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::PrintSupport PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5PrintSupport_LIB_DEPENDENCIES}${_list_sep}${_Qt5PrintSupport_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5PrintSupport_LIB_DEPENDENCIES}${_list_sep}${_Qt5PrintSupport_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::PrintSupport)
    ...
    set(Qt5PrintSupport_PRIVATE_INCLUDE_DIRS
        "${_qt5PrintSupport_install_prefix}/include/QtPrintSupport/5.6.0"
        "${_qt5PrintSupport_install_prefix}/include/QtPrintSupport/5.6.0/QtPrintSupport"
    )
    ...
    foreach(_module_dep ${_Qt5PrintSupport_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5PrintSupport_FIND_VERSION_EXACT}
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5PrintSupport_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5PrintSupport_PLUGINS)
            if (matched)
    ...
    macro(_populate_PrintSupport_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5PrintSupport_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_PrintSupport_target_properties(RELEASE "Qt5PrintSupport.dll" "libQt5PrintSupport.dll.a" )



    _populate_PrintSupport_target_properties(DEBUG "Qt5PrintSupportd.dll" "libQt5PrintSupportd.dll.a" )
