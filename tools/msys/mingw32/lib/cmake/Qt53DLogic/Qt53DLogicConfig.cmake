...
macro(_populate_3DLogic_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt53DLogic_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::3DLogic PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt53DLogic_LIB_DEPENDENCIES}${_list_sep}${_Qt53DLogic_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt53DLogic_LIB_DEPENDENCIES}${_list_sep}${_Qt53DLogic_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::3DLogic)
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt53DLogic_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt53DLogic_PLUGINS)
            if (matched)
    ...
    macro(_populate_3DLogic_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        set(imported_location "${_qt53DLogic_install_prefix}/share/qt5/plugins/${PLUGIN_LOCATION}")
        _qt5_3DLogic_check_file_exists(${imported_location})
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt53DLogic_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_3DLogic_target_properties(RELEASE "Qt53DLogic.dll" "libQt53DLogic.dll.a" )



    _populate_3DLogic_target_properties(DEBUG "Qt53DLogicd.dll" "libQt53DLogicd.dll.a" )
