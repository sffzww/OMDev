...
set(Qt5AxContainer_VERSION_STRING 5.6.0)
...
macro(_populate_AxContainer_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5AxContainer_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::AxContainer PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5AxContainer_LIB_DEPENDENCIES}${_list_sep}${_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5AxContainer_LIB_DEPENDENCIES}${_list_sep}${_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
    ...
    foreach(_module_dep ${_Qt5AxContainer_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5AxContainer_FIND_VERSION_EXACT}
...
# plugin, each time, appending to _Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES
...
macro(macro_process_prl_file prl_file_location Configuration)
    if (EXISTS "${prl_file_location}")
        ...
        string(REGEX REPLACE "\\$\\$\\[QT_INSTALL_LIBS\\]" "${_qt5AxContainer_install_prefix}/lib" static_depends "${static_depends}")
        if (_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES)
            set(_list_sep ";")
        endif()
        set(_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES "${_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES}${_list_sep}${static_depends}")
    endif()
endmacro()
...
macro(macro_process_plugin_prl_file prl_file_location Configuration LIB_DIRECTORY LIB_LOCATION)
    ...
    set(_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES "${_Qt5AxContainer_STATIC_${Configuration}_LIB_DEPENDENCIES}${_list_sep}${_qt5AxContainer_install_prefix}/${LIB_DIRECTORY}/${LIB_LOCATION}")
    # .. then its dependencies.
    macro_process_prl_file(${prl_file_location} ${Configuration})
endmacro()

    macro_process_prl_file("${_qt5AxContainer_install_prefix}/lib/Qt5AxContainerd.prl" DEBUG)
    macro_process_prl_file("${_qt5AxContainer_install_prefix}/lib/Qt5AxContainer.prl" RELEASE)
...
# Excerpt from: end
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5AxContainer_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5AxContainer_PLUGINS)
            if (matched)
    ...
    macro(_populate_AxContainer_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        set(imported_location "${_qt5AxContainer_install_prefix}/share/qt5/plugins/${PLUGIN_LOCATION}")
        _qt5_AxContainer_check_file_exists(${imported_location})
        ...
        set_property(TARGET Qt5::AxContainer APPEND PROPERTY STATIC_PLUGINS ${Plugin})
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5AxContainer_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
