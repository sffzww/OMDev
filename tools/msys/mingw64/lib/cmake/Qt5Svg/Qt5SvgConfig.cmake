...
set(Qt5Svg_VERSION_STRING 5.6.0)
...
macro(_populate_Svg_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5Svg_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::Svg PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5Svg_LIB_DEPENDENCIES}${_list_sep}${_Qt5Svg_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5Svg_LIB_DEPENDENCIES}${_list_sep}${_Qt5Svg_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::Svg)
    ...
    set(Qt5Svg_PRIVATE_INCLUDE_DIRS
        "${_qt5Svg_install_prefix}/include/QtSvg/5.6.0"
        "${_qt5Svg_install_prefix}/include/QtSvg/5.6.0/QtSvg"
    )
    ...
    foreach(_module_dep ${_Qt5Svg_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5Svg_FIND_VERSION_EXACT}
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5Svg_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5Svg_PLUGINS)
            if (matched)
    ...
    macro(_populate_Svg_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        set(imported_location "${_qt5Svg_install_prefix}/share/qt5/plugins/${PLUGIN_LOCATION}")
        _qt5_Svg_check_file_exists(${imported_location})
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5Svg_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_Svg_target_properties(RELEASE "Qt5Svg.dll" "libQt5Svg.dll.a" )



    _populate_Svg_target_properties(DEBUG "Qt5Svgd.dll" "libQt5Svgd.dll.a" )
