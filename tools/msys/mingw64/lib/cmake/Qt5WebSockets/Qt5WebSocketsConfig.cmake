...
set(Qt5WebSockets_VERSION_STRING 5.6.0)
...
macro(_populate_WebSockets_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5WebSockets_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::WebSockets PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5WebSockets_LIB_DEPENDENCIES}${_list_sep}${_Qt5WebSockets_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5WebSockets_LIB_DEPENDENCIES}${_list_sep}${_Qt5WebSockets_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::WebSockets)
    ...
    set(Qt5WebSockets_PRIVATE_INCLUDE_DIRS
        "${_qt5WebSockets_install_prefix}/include/QtWebSockets/5.6.0"
        "${_qt5WebSockets_install_prefix}/include/QtWebSockets/5.6.0/QtWebSockets"
    )
    ...
    foreach(_module_dep ${_Qt5WebSockets_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5WebSockets_FIND_VERSION_EXACT}
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5WebSockets_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5WebSockets_PLUGINS)
            if (matched)
    ...
    macro(_populate_WebSockets_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        set(imported_location "${_qt5WebSockets_install_prefix}/share/qt5/plugins/${PLUGIN_LOCATION}")
        _qt5_WebSockets_check_file_exists(${imported_location})
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5WebSockets_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_WebSockets_target_properties(RELEASE "Qt5WebSockets.dll" "libQt5WebSockets.dll.a" )



    _populate_WebSockets_target_properties(DEBUG "Qt5WebSocketsd.dll" "libQt5WebSocketsd.dll.a" )
