...
set(Qt5Gui_VERSION_STRING 5.6.0)
...
macro(_populate_Gui_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5Gui_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::Gui PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5Gui_LIB_DEPENDENCIES}${_list_sep}${_Qt5Gui_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5Gui_LIB_DEPENDENCIES}${_list_sep}${_Qt5Gui_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::Gui)
    ...
    set(Qt5Gui_PRIVATE_INCLUDE_DIRS
        "${_qt5Gui_install_prefix}/include/QtGui/5.6.0"
        "${_qt5Gui_install_prefix}/include/QtGui/5.6.0/QtGui"
    )
    ...
    foreach(_module_dep ${_Qt5Gui_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5Gui_FIND_VERSION_EXACT}
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5Gui_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5Gui_PLUGINS)
            if (matched)
    ...
    macro(_populate_Gui_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5Gui_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_Gui_target_properties(RELEASE "Qt5Gui.dll" "libQt5Gui.dll.a" )



    _populate_Gui_target_properties(DEBUG "Qt5Guid.dll" "libQt5Guid.dll.a" )
