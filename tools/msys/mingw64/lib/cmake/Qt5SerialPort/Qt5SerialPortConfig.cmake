...
set(Qt5SerialPort_VERSION_STRING 5.6.0)
...
macro(_populate_SerialPort_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION)
    ...
    if (_Qt5SerialPort_LIB_DEPENDENCIES)
        set(_list_sep ";")
    endif()
    set_target_properties(Qt5::SerialPort PROPERTIES
        "INTERFACE_LINK_LIBRARIES_${Configuration}" "${_Qt5SerialPort_LIB_DEPENDENCIES}${_list_sep}${_Qt5SerialPort_STATIC_${Configuration}_LIB_DEPENDENCIES}"
        "IMPORTED_LOCATION_${Configuration}" ${imported_location}
        # For backward compatibility with CMake < 2.8.12
        "IMPORTED_LINK_INTERFACE_LIBRARIES_${Configuration}" "${_Qt5SerialPort_LIB_DEPENDENCIES}${_list_sep}${_Qt5SerialPort_STATIC_${Configuration}_LIB_DEPENDENCIES}"
    )
...
if (NOT TARGET Qt5::SerialPort)
    ...
    set(Qt5SerialPort_PRIVATE_INCLUDE_DIRS
        "${_qt5SerialPort_install_prefix}/include/QtSerialPort/5.6.0"
        "${_qt5SerialPort_install_prefix}/include/QtSerialPort/5.6.0/QtSerialPort"
    )
    ...
    foreach(_module_dep ${_Qt5SerialPort_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.6.0 ${_Qt5SerialPort_FIND_VERSION_EXACT}
    ...
    file(GLOB pluginTargetsMaybe "${CMAKE_CURRENT_LIST_DIR}/Qt5SerialPort_*.cmake")
    unset(pluginTargets)
    if (pluginTargetsMaybe)
        foreach(pluginTarget ${pluginTargetsMaybe})
            file(STRINGS ${pluginTarget} matched REGEX Qt5SerialPort_PLUGINS)
            if (matched)
    ...
    macro(_populate_SerialPort_plugin_properties Plugin Configuration PLUGIN_LOCATION)
        ...
        set(imported_location "${_qt5SerialPort_install_prefix}/share/qt5/plugins/${PLUGIN_LOCATION}")
        _qt5_SerialPort_check_file_exists(${imported_location})
        ...
        string(REGEX REPLACE "^lib(.*)\\.a" "${_qt5SerialPort_install_prefix}/share/qt5/plugins/${_plugindirname}/\\1.prl" plugin_prl_file_location ${_pluginname})
    ...
    _populate_SerialPort_target_properties(RELEASE "Qt5SerialPort.dll" "libQt5SerialPort.dll.a" )



    _populate_SerialPort_target_properties(DEBUG "Qt5SerialPortd.dll" "libQt5SerialPortd.dll.a" )
